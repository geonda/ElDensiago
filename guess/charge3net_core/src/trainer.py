...
class Trainer:
    def __init__(
       ...
       # Select device with MPS support
        if torch.backends.mps.is_available():
            device = torch.device("mps")
        elif torch.cuda.is_available():
            # If you want to support cuda too
            device = torch.device(f"cuda:{gpu_id if gpu_id is not None else 0}")
        else:
    ...
    def test(self, test_dl, cube_dir=None, max_predict_batch_probes=2500):
        ...
        if self.local_rank == 0 and self.global_rank == 0:
            preds = [torch.load(f, map_location=self.local_rank, weights_only=True) for f in tmp_nmape_dir.iterdir() if "pred" in f.name]
        ...
        if batch["num_probes"] > max_predict_batch_probes:            
            ...
            for i, sub_batch in enumerate(pred_utils.split_batch(batch, max_predict_batch_probes)):
                ...
                if atom_repr is None:
                    atom_repr = self.model.atom_model(sub_batch)
                if isinstance(self.model, PainnDensityModel):
                    # PaiNN takes (scalar, vector) tuple as two args
                    outputs = self.model.probe_model(sub_batch, *atom_repr)
                else:
                    outputs = self.model.probe_model(sub_batch, atom_repr)
