...
import guess.charge3net_core.src.charge3net.data.layer as layer
...
class E3AtomRepresentationModel(nn.Module):
    ...
    def forward(self, input_dict):
        # input_dict["num_probes"].device=torch.device("cpu")
...
def calc_edge_vec(
    ...
    (modified from guess.charge3net_core.src.data.layer.calc_distance)
    ...
    device = cells.device
    if device.type == 'mps':
        unitcell_repeat = torch.repeat_interleave(cells.cpu(), splits.cpu(), dim=0).to(device)
    else:
        unitcell_repeat = torch.repeat_interleave(cells, splits, dim=0)
